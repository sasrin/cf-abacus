...
const report = (name, time, latency, err, timeout, reject, circuit, queued,
inflight, batched) => {
  calls.emit('message', {
    metrics: {
      call: {
...
const rollCounts = (buckets, time) => {
  return roll(buckets, time, 3600000, 10, () => ({
      i: 0,
      ok: 0,
      errors: 0,
      timeouts: 0,
...
const rollLatencies = (buckets, time) => {
  return roll(buckets, time, 3600000, 6, () => ({
      i: 0,
      latencies: []
  }));
};
...
const rollHealth = (buckets, time, counts, queue, batch) => {
...
const stats = (name, time, roll) => {
  ...
  return roll !== false ? (() => {
    ...
    return {
      ...
      queue: rollQueue(s.queue, time),
      batch: rollBatch(s.batch, time),
      lock: rollLock(s.lock, time),
      circuit: s.circuit
    };
  })() : {
    name: name,
...
const accumulateStats = (name, time, latency, err, timeout,
  reject, circuit, queued, inflight, batched) => {
    debug('Accumulating stats for function %s', name);
    debug('latency %d, err %s, timeout %d, reject %s, circuit %s, queued %s,' +
      'inflight %s, batched %s', latency, err, timeout, reject, circuit, queued,
      inflight, batched);
    ...
    const queue = rollQueue(s ? s.queue : [], time);
    const updateQueue = (q) => {
      q.ok = q.ok + (!err && !timeout && !reject ? 1 : 0);
      q.errors = q.errors + (err ? 1 : 0);
      q.queued = queued ? queued : 0;
      q.inflight = inflight ? inflight : 0;
      debug('%d ok, %d errors, %d queued, %d inflight, %d queue buckets',
        q.ok, q.errors, q.queued, q.inflight, queue.length);
    };
    updateQueue(last(queue));
  ...
  if(msg.metrics) {
    ...
    if(msg.metrics.call) {
      ...
      accumulateStats(c.name, c.time, c.latency, c.error, c.timeout, c.reject,
        c.circuit, c.queued, c.inflight, c.batched);
    }
