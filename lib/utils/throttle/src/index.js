...
const throttlefn = function(fn, o) {
  ...
  const opt = defaults(o ? o : {}, { name: fn.name || fn.fname, max: 5000 });
    ...
    try {
      return fcb.apply(null, initial(args).concat([function(err, val) {
        ...
        try {
          ...
          next(Date.now() - start, !!err);
        }
      }]));
    }
    catch(e) {
      // Schedule the execution of the next queued call
      console.log('Exception at throttle %o', e);
  ...
  const next = (time, failed) => {
    ...
    perf.report(opt.name, Date.now(), time, failed, 0, 0, 0, queue.length,
      running, 0);
  };
